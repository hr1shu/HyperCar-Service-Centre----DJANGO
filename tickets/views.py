...
class ProcView(View):
    @staticmethod
    def get(request):
        que = {"change_oil": len(TicketView.line_of_cars['change_oil']),
                      "inflate_tires": len(TicketView.line_of_cars['inflate_tires']),
                      "diagnostic": len(TicketView.line_of_cars['diagnostic'])}
        return render(request, 'tickets/processing.html', {"queue": que})

    @staticmethod
    def post(request):
        for act in TicketView.line_of_cars:
            if TicketView.line_of_cars[act]:
                global active_client_identifier
                active_client_identifier = TicketView.line_of_cars[act].popleft()
                break
            active_client_identifier = ''
